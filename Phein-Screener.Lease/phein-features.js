...
(function() {
    ...
    function playToggleSound(toggleId, turningOn) {
        // Bug 4 fix: respect the sound toggle. Without this check, toggle sounds
        // play even when the user has muted the system via the Sound button.
        if (window.pheinSoundsEnabled === false) return;
    ...
    // =============================================
    // FEATURE 8: CANDIDATE COMPARISON
    // =============================================
    window.toggleComparisonSelect = function(candidateKey) {
        if (state.comparisonSelected.has(candidateKey)) {
            ...
            if (state.comparisonSelected.size >= 6) {
    ...
    function showCompareLimitPopup() {
        ...
        overlay.style.cssText = 'position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.5);backdrop-filter:blur(3px);z-index:7000;display:flex;justify-content:center;align-items:center;';
        var cardBg = isDark ? '#1e293b' : '#ffffff';
        var titleColor = isDark ? '#f1f5f9' : '#0f172a';
        var textColor = isDark ? '#94a3b8' : '#64748b';
        var iconBg = isDark ? 'rgba(239,68,68,0.15)' : '#fee2e2';
        var iconBorder = isDark ? 'rgba(239,68,68,0.3)' : '#fecaca';
        overlay.innerHTML =
            '<div style="background:' + cardBg + ';border-radius:18px;padding:2rem 2.25rem;max-width:400px;width:92%;box-shadow:0 25px 60px rgba(0,0,0,0.3);animation:modalSlideIn 0.3s ease-out;text-align:center;">' +
                '<div style="width:56px;height:56px;background:' + iconBg + ';border:2px solid ' + iconBorder + ';border-radius:50%;display:flex;align-items:center;justify-content:center;margin:0 auto 1.25rem;">' +
                    '<svg viewBox="0 0 24 24" fill="none" stroke="#ef4444" stroke-width="2" style="width:28px;height:28px;"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>' +
                '</div>' +
                '<h3 style="font-size:1.15rem;font-weight:700;color:' + titleColor + ';margin-bottom:0.5rem;">Comparison Limit Reached</h3>' +
                '<p style="font-size:0.88rem;color:' + textColor + ';line-height:1.6;margin-bottom:1.5rem;">You can compare a maximum of <strong style="color:#ef4444;">6 candidates</strong> at a time. Unselect one before adding another.</p>' +
                '<button onclick="document.getElementById(\'compare-limit-popup\').remove()" style="background:var(--primary);color:white;border:none;padding:0.65rem 2rem;border-radius:10px;font-weight:600;cursor:pointer;font-size:0.9rem;box-shadow:0 4px 12px rgba(37,99,235,0.3);transition:all 0.2s;" onmouseover="this.style.background=\'var(--primary-dark)\'" onmouseout="this.style.background=\'var(--primary)\'">Got It</button>' +
            '</div>';
