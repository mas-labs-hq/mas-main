 ...
 * ==========================================
 * Centralized Formspree alert system with DIAGNOSTIC PANEL.
 *
 ...
 *
 * DIAGNOSTIC: A floating panel shows real-time status of every attempt.
 *   Run PheinFormspree.test() from console to test.
 *   Run PheinFormspree.showPanel() to show the diagnostic panel.
 *
...
(function (window, document) {
  ...
  // ===============================================================
  // DIAGNOSTIC PANEL — shows real-time alert status
  // ===============================================================
  var panelEl = null;
  var panelLog = [];
  ...
  function logToPanel(message, type) {
    ...
    var p = ensurePanel();
    if (p) {
      var logEl = p.querySelector('#phein-fs-panel-log');
      if (logEl) {
  ...
  function showToast(message, type) {
    if (!document.body) {
      if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', function () { showToast(message, type); });
        return;
      }
      return;
    }
  ...
  function sendViaHiddenForm(fields) {
    ...
    try {
      ...
      // Listen for iframe load to detect if Formspree responded
  ...
  function send(subject, fields, options) {
    ...
    logToPanel('Protocol: ' + window.location.protocol, 'info');
      ...
      if (fetchOk) {
        ...
        if (options.showToast !== false) {
          if (beaconSent) {
            ...
            showToast('ALERT FAILED — check diagnostic panel', 'error');
          }
        }
      }
    });
  ...
  // ===============================================================
  // TEST FUNCTION — run PheinFormspree.test() from console
  // ===============================================================
  function test() {
    showPanel();
    logToPanel('══════════════════════════════════════', 'info');
    ...
    if (window.location.protocol === 'file:') {
      logToPanel('WARNING: You are opening from file:// protocol. Formspree may reject submissions from file:// origin. Test from a web server (http://localhost) for best results.', 'warn');
    }
    ...
    return 'Test dispatched. Watch the diagnostic panel (top-right) for real-time status. Check your Formspree inbox within 30 seconds.';
  }
