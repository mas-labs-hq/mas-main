...
// =====================================================
var SLOT_ADS = [
    { image: 'ad-images/ad1.jpg', url: 'https://www.mortappsstudios.com', alt: 'MortApps Studios', duration: '5s', cta: 'Visit Website', tagline: 'Premium Web & Software Development in Kenya' },
    { image: 'ad-images/ad2.jpg', url: 'https://example.com', alt: 'Advertisement', duration: '10s', cta: 'Learn More', tagline: 'Taste the Feeling' },
    { image: 'ad-images/ad3.jpg', url: 'https://example.com', alt: 'Advertisement', duration: '5s', cta: 'Learn More', tagline: 'Your Trusted Partner' }
];
...
function bindNav() {
    ...
    if (!nav) return;
    // Use requestAnimationFrame to throttle scroll handler for smoothness
    var ticking = false;
...
function bindSearch() {
    ...
    input.addEventListener('input', function(e) {
        ...
        // More aggressive debounce for smoother typing on mobile
        debounceTimer = setTimeout(function() {
            currentFilters.search = e.target.value.toLowerCase().trim();
            // Use requestAnimationFrame for smoother DOM updates
            requestAnimationFrame(renderHouses);
        }, 150);
    }, { passive: true });
}
...
function renderHouseCard(house, idx) {
    ...
    if (house.images && house.images.length > 0) {
        // Performance: lazy load + async decode + width/height to prevent layout shift
        imgHtml = '<img src="' + house.images[0] + '" alt="' + escapeHtml(house.title) + '" loading="lazy" decoding="async" width="400" height="300" onerror="this.parentElement.innerHTML=getPlaceholderHtml()">';
    } else {
    ...
    var delay = Math.min(idx * 0.04, 0.3).toFixed(2);
...
// =====================================================
// HOUSE DETAIL MODAL - SMOOTH SWIPE CAROUSEL
// =====================================================
var carouselTouchStartX = 0;
var carouselTouchCurrentX = 0;
...
function openHouseModal(id) {
    ...
    // Build carousel with all images preloaded (smooth swipe, no lag)
    var carouselHtml = '';
    if (images.length > 0) {
        var slidesHtml = images.map(function(imgSrc, i) {
            return '<div class="carousel-slide">' +
                '<img src="' + imgSrc + '" alt="' + escapeHtml(house.title) + ' - Photo ' + (i + 1) + '" loading="' + (i === 0 ? 'eager' : 'lazy') + '" decoding="async" onerror="this.parentElement.innerHTML=getDetailPlaceholderHtml()">' +
            '</div>';
        }).join('');
        carouselHtml = '<div class="carousel-track" id="carouselTrack">' + slidesHtml + '</div>';
    } else {
        carouselHtml = getDetailPlaceholderHtml();
    }
    ...
    // Build dots
    if (images.length > 1) {
        var dots = body.querySelector('#carouselDots');
        if (dots) {
...
function setupCarouselSwipe() {
    ...
    function onMove(e) {
        ...
        var slidePercent = (diff / carouselWidth) * 100;
        track.style.transform = 'translateX(calc(-' + (currentImageIndex * 100) + '% + ' + slidePercent + '%))';
        if (e.touches) e.preventDefault();
    }
    ...
    function onEnd() {
        ...
        var diff = currentX - startX;
        var threshold = carouselWidth / 4; // 25% swipe threshold
        if (diff < -threshold) {
            nextImage();
...
function prevImage() {
    ...
    updateCarousel();
    // Preload next image for instant swipe
    var nextIdx = (currentImageIndex + 1) % currentHouse.images.length;
    preloadImage(currentHouse.images[nextIdx]);
}
...
function updateCarousel() {
    ...
    if (!track || !currentHouse.images) return;
