...
async function saveExam() {
  ...
  // Demo: Enforce exam cap when creating new exam
  if (!editId && SV.IS_DEMO) {
    const examCount = await db.count('exams');
    if (examCount >= SV.DEMO_EXAM_CAP) {
      showExamCapWarning();
      return;
    }
  }
...
async function loadRankingsTable() {
  ...
  ranked.forEach(r => {
    ...
    if (!stream) {
      const classes = (typeof db !== 'undefined') ? (await db.getSetting('classes') || []) : [];
