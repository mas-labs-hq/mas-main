...
const app = {
    ...
    bindEvents: function() {
        ...
        // NEW: auto-save time settings when admin edits them, so the
        // scanner always uses the live threshold value (fixes the
        // "9:21 marked Late against 9:30 threshold" bug — that bug was
        // caused by the admin changing the input but not clicking Save).
        var lateInput = document.getElementById('late-threshold');
        var endInput = document.getElementById('work-end-time');
        var otInput = document.getElementById('overtime-threshold');
        if (lateInput) lateInput.addEventListener('change', () => admin.autoSaveTimeSettings());
        if (endInput) endInput.addEventListener('change', () => admin.autoSaveTimeSettings());
        if (otInput) otInput.addEventListener('change', () => admin.autoSaveTimeSettings());
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    neuralEngine.loadModels().then(async () => {
        loadingStatus.textContent = 'Warming up neural engine...';
        await neuralEngine.warmUp();
