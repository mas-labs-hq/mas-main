...
var admin = {
    ...
    saveSettings: function() {
        ...
        // DEMO: Block password update (premium feature)
        if (pinInput.value) {
    ...
    /**
     * Auto-save the late threshold, work-end time, and overtime threshold
     * whenever the admin changes them — no need to click "Save Settings".
     *
     * FIXES: The original "9:21 marked Late against 9:30 threshold" bug
     * was caused by admins changing the threshold input but never clicking
     * Save Settings, so the scanner was still using the stale default
     * (09:00) — and 9:21 IS late against 09:00. Now the live value is
     * persisted immediately on change.
     */
    autoSaveTimeSettings: function() {
        var lateInput = document.getElementById('late-threshold');
        var endInput = document.getElementById('work-end-time');
        var overtimeInput = document.getElementById('overtime-threshold');
        var changed = false;

        if (lateInput && lateInput.value && lateInput.value !== db.data.lateThreshold) {
            db.data.lateThreshold = lateInput.value;
            changed = true;
        }
        if (endInput && endInput.value && endInput.value !== db.data.workEndTime) {
            db.data.workEndTime = endInput.value;
            changed = true;
        }
        if (overtimeInput && overtimeInput.value) {
            var ot = parseInt(overtimeInput.value, 10) || 8;
            if (ot !== db.data.overtimeThreshold) {
                db.data.overtimeThreshold = ot;
                changed = true;
            }
        }
        if (changed) db.save();
    ...
    startFaceRegistration: function() {
        ...
        // DEMO: 3 employee cap
        if (typeof DEMO_MAX_EMPLOYEES !== 'undefined' && db.data.employees.length >= DEMO_MAX_EMPLOYEES) {
            utils.showToast('Demo limit reached! Max ' + DEMO_MAX_EMPLOYEES + ' employees. Upgrade for unlimited.', 5000, 'warning');
            return;
        }
    ...
    captureLoop: function() {
        ...
        // Bumped from 3 → 5 samples. More samples = more robust matching
        // when the employee later checks in from a different camera
        // (front↔rear) or under different lighting.
        var SAMPLES_REQUIRED = 5;

        if (neuralEngine.lastDetection) {
            this.tempDescriptors.push(neuralEngine.lastDetection.descriptor);
            if (this.tempDescriptors.length < SAMPLES_REQUIRED) {
                statusMsg.textContent = 'Capturing... ' + this.tempDescriptors.length + '/' + SAMPLES_REQUIRED;
                setTimeout(function() { self.captureLoop(); }, 600);
            } else {
                this.isRegistering = false;
                // Release the map visible flag after registration completes
                neuralEngine._keepMapVisible = false;

                // Capture profile photo using camera-aware helper
                // (so rear-camera registrations don't come out mirrored)
                this.tempPhoto = camera.captureSnapshot ? camera.captureSnapshot() : null;
                ...
                camera.stop();
                app.nav('dashboard');
                document.getElementById('reg-status').textContent = SAMPLES_REQUIRED + ' face samples captured! Ready to save.';
    ...
    checkBackupReminder: function() {
        // DEMO: No backup reminder in demo version — feature is premium anyway
        var banner = document.getElementById('backup-reminder-banner');
        if (banner) banner.style.display = 'none';
    },

    dismissBackupReminder: function() {
        // DEMO: No backup reminder in demo version
    },
