...
// =====================================================
// BIOATTEX DEMO — NEURAL ENGINE (v2.1 — POLISHED)
// Fixes:
...
// Demo-only features preserved:
//   • warmUp() — pre-compiles WebGL shaders during loading screen
//   • Cross-camera fallback — relaxed-threshold retry using cached preview
// =====================================================
...
var neuralEngine = {
    ...
    _lastPreviewMatchEmp: null,    // DEMO: cached preview match (used by scanner cross-camera fallback)
    _hadDrawing: false,
    ...
    // Also stashes the best match in _lastPreviewMatchEmp so the
    // scanner can use it as a relaxed-threshold fallback.
...
var camera = {
    ...
    start: async function() {
        ...
        try {
            ...
            // Watch for video / wrapper dimension changes (device rotation, browser resize, layout shifts)
     ...
     * produce a flipped, wrong-looking image).
     */
...
var scanner = {
    ...
    initiateScan: async function(type) {
        ...
        // DEMO-SPECIFIC: Cross-camera fallback. If the strict threshold
        // didn't match but the background preview recently recognised
        // someone, retry with a relaxed 0.7 threshold. This catches
        // cases where the user registered with one camera and is now
        // checking in with the other — the descriptor distance will be
        // slightly larger due to lens/skin-tone differences.
     ...
     * complaint): the threshold often wasn't being saved when the admin
     * changed it. Now we read the LIVE input value first (falling back
     * to the saved value), so unsaved changes still take effect. We
     * also normalise the threshold string defensively (trim, leading
     * zeros) in case it was hand-edited in localStorage.
     *
     * The demo's original code also used `>=` for minutes which meant
     * 9:30:00 vs a 9:30 threshold = Late. That's wrong (HR convention
     * is that arriving AT the threshold time is "on time"). Now uses
     * `>` so equality = On Time (grace).
     */
